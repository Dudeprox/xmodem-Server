...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        if (state == send_block) {
            ...
            write(soc, &char_block, 1); // The current block number
            char_block = 255 - char_block;
            write(soc, &char_block, 1); // The inverse of the current block number
            write(soc, buf, 128); // The actual payload. This is always 128 bytes
            unsigned short crc = crc_message(XMODEM_KEY, buf, 128);
            printf("Client wants to send crc %x\n", crc);
            char_block = crc >> 8;
            printf("Client sends crc first byte %x\n", char_block);
            write(soc, &char_block, 1); // The high byte of the CRC16 of the payload.
            char_block = crc; 
            printf("Client sends crc second byte %x\n", char_block);
            write(soc, &char_block, 1); // The low byte of the CRC16 of the payload.
